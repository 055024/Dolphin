#!/usr/bin/env python3
"""
Evaluate test-docs Dolphin results from batch_results/recognition_json/
"""
...
def evaluate_document(doc_path, json_path):
    ...
    try:
        ...
        print(f"   Substitutions: {wer_details['substitutions']}, Deletions: {wer_details['deletions']}, Insertions: {wer_details['insertions']}")
...
def main():
    # Find all JSON files in batch_results/recognition_json
    json_files = glob.glob('./batch_results/recognition_json/*.json')
    json_files.sort()
    
    print(f"\n{'#'*80}")
    print(f"EVALUATING TEST-DOCS RESULTS")
    print(f"{'#'*80}")
    ...
    for json_path in json_files:
        ...
        # Find corresponding PDF in test-docs folder
        doc_path = f'./test-docs/{base_name}.pdf'
        
        if not os.path.exists(doc_path):
            print(f"\n⚠️  Could not find source document: {doc_path}")
            continue
    ...
    if successful > 0:
        ...
        print(f"Summary Statistics ({successful} documents):")
